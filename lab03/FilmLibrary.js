...
export default function FilmLibrary() {
    ...
    this.getWatchedLastMonth = () => {
        return new Promise((resolve, reject) => {
            const query = 'SELECT * FROM films WHERE watchDate > ? AND watchDate <= ?';
            const now = dayjs();
            const prev = now.add(-1, 'month');
            db.all(query, [prev.format('YYYY-MM-DD'), now.format('YYYY-MM-DD')], (err, rows) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(mapRowsToFilms(rows));
