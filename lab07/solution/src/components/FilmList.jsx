...
export default function FilmList(props) {
    ...
    return (
        <ListGroup id="films-list" variant="flush">
            {props.films.map((film) => <FilmInList key={film.id} filmData={film} updateFilm={props.updateFilm} deleteFilm={props.deleteFilm}/>)}
        </ListGroup>
    );
}
            ...
            <Col xs={8} xl={3} className="actions-container text-end">
                <div className="rating">
                    <Rating rating={props.filmData.rating} maxStars={5}
                        updateRating={(newRating) => props.updateFilm({ ...props.filmData, rating: newRating })}/>
                </div>
